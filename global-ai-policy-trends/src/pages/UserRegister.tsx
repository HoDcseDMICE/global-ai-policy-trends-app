...
export default function UserRegister() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mockUser = { username: username || 'User', role: 'user' as const };
      login(mockUser, 'mock-token-user');
      navigate('/');
    } catch (err: any) {
      ...
      >
        <div className="glass-panel p-8 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">Create a User Account</p>
          </div>
          ...
          <p className="mt-8 text-center text-sm text-slate-400">
            ...
            <Link to="/login/user" className="text-brand-primary hover:text-white font-medium transition-colors">
              Sign in
            </Link>
          </p>
        </div>
